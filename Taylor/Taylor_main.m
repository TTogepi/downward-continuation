...
%% Ref: Blakely 1995.
%%
function [sum_f_con]=Taylor_main(f,expand,Cell,z,n_taylor)
...
Fourier_fcon = zeros(n_taylor,mnew,nnew);
f_con_temp = zeros(mnew,nnew);
f_con =  zeros(n_taylor,m,n);

for i = 1 : n_taylor
    
    check =  (K.^(i -1));
    Fourier_fcon(i,:,:) = (K.^(i -1)) .* U; 
...
%% 
sum_f_con = zeros(m,n);
for i_taylor = 1 : n_taylor 
    
    for i = 1 : m
    for j = 1 : n
        sum_f_con(i,j)  =  sum_f_con(i,j) + z ^ (i_taylor - 1) / factorial(i_taylor - 1) * f_con(i_taylor, i, j);
    end
    end
